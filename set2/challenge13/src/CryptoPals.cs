...
namespace CryptoPals {
	...
	public static byte[] EncryptECB( byte[] plaindata, byte[] secretkey ){
	    ...
	    byte[] paddeddata = PKCS.Pad( plaindata, 16 );
	...
	public static byte[] DecryptECB( byte[] cipherdata, byte[] secretkey ){
	    ...
	    return PKCS.UnPad( decryptor.TransformFinalBlock( cipherdata, 0, cipherdata.Length ), 16 );
	}
	...
	public static byte[] UnPad( byte[] inputblock, int blocksize ){
	    byte lastbyte = inputblock[ inputblock.Length -1 ];
	    int outputlength = inputblock.Length - lastbyte; //e.g. if padding was \x04 then new length == inputlength - 4
	    
	    byte[] output = new byte[ outputlength ];

	    Buffer.BlockCopy(inputblock, 0, output, 0, outputlength );
